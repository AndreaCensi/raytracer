...
/*	options_string(ops, "poses", &p.file_map, "-", "List of poses");*/
	...
	double pose[2] = {0.1,0.1}; double theta = 3;
	
	{
		LDP ld = ld_alloc_new(p.nrays);
		
		ld->true_pose[0] = pose[0];
		ld->true_pose[1] = pose[1];
		ld->true_pose[2] = theta;
		...
		for(int i=0;i<ld->nrays;i++) {
			...
			if(env.ray_tracing(pose, ld->theta[i] + theta, rho, alpha, &stuff_id) && (rho<p.max_reading)) {
				ld->valid[i] = 1;
				ld->readings[i] = rho;
				
				ld->true_alpha[i] = normalize_0_2PI(alpha-theta);
...
/*	lh_entry*entry;
	lh_foreach(json_object_get_object(jo), entry) {
